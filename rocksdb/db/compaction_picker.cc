...
namespace rocksdb {
	...
	/*bool IsOverlappingKeyRanges(const Comparator* c, std::string asmallestkey, std::string alargestkey, std::string bsmallestkey, std::string blargestkey) {
		  if (c->Compare(asmallestkey, bsmallestkey) >= 0) {
			      if (c->Compare(asmallestkey, blargestkey) <= 0) {
				            return true;
					        }
			        } else if (c->Compare(alargestkey, bsmallestkey) >= 0) {
					    return true;
					      }
		    if (c->Compare(alargestkey, blargestkey) <= 0) {
			        if (c->Compare(alargestkey, bsmallestkey) >= 0) {
					      return true;
					          }
				  } else if (c->Compare(asmallestkey, blargestkey) <= 0) {
					      return true;
					        }
		      return false;
	}*/
	...
	namespace {
		...
		Compaction* LevelCompactionBuilder::GetCompaction() {
			...
			/*const Comparator* thecomparator = vstorage_->InternalComparator()->user_comparator();

			std::vector<int> compacted_files;

			if(c->start_level() == 1 && c->output_level() == 2 && c->num_input_levels() != 1) {

				std::cout << "************ Compaction Event LEVEL 1 BEGINN ******************" << std::endl;
				for(size_t i=0;i< c->num_input_files(0); i++){
					compacted_files.push_back(c->input(0,i)->fd.GetNumber());
				}
				for(size_t i=0;i< c->num_input_files(1); i++){
				}

				double current_size = 0;
				double current_size2 = 0;
				double total_size = 0;
				std::string file_line;
				std::string file_line2;

				for(int i=0; i < vstorage_->NumLevelFiles(1); i++) {

					total_size = 0;
					current_size2 = 0;
					current_size = 0;
					current_size = vstorage_->LevelFiles(1)[i]->fd.GetFileSize();

					for(int j=0; j < vstorage_->NumLevelFiles(2); j++) {

						current_size2 = vstorage_->LevelFiles(2)[j]->fd.GetFileSize();

						if(IsOverlappingKeyRanges(thecomparator, vstorage_->LevelFiles(1)[i]->smallest.user_key().ToString(),
									vstorage_->LevelFiles(1)[i]->largest.user_key().ToString(),
									vstorage_->LevelFiles(2)[j]->smallest.user_key().ToString(),
									vstorage_->LevelFiles(2)[j]->largest.user_key().ToString())) {

							total_size += current_size2;

						}
					}


					double WA = (double)(((double)total_size) / (double)current_size);

					if(std::find(compacted_files.begin(), compacted_files.end(), vstorage_->LevelFiles(1)[i]->fd.GetNumber()) != compacted_files.end()){
						std::cout << " Filename "<< vstorage_->LevelFiles(1)[i]->fd.GetNumber() << " Level 1 " << " WA : "<< WA << " SELECTED " << std::endl;
					} else {
						std::cout << " Filename "<< vstorage_->LevelFiles(1)[i]->fd.GetNumber() << " Level 1 " << " WA : "<< WA << std::endl;
					}

				}
				std::cout << "************ Compaction Event LEVEL 1 END ******************" << std::endl;
			}*/
