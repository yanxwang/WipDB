...
DEFINE_int64(range, -1, "Number of key/values to place in database");
...
DEFINE_int32(rwdelay, 10, "rw delay in us");
 ...
 public:
  ...
  void PrintThreadStatus() {
    ...
    for (auto ts : thread_list) {
      fprintf(stdout, "%18" PRIu64 " %10s %12s %20s %13s %45s %12s",
          ts.thread_id,
...
struct ThreadState {
  ...
  /* implicit */ ThreadState(int index)
      : tid(index),
        rand((FLAGS_seed ? FLAGS_seed : 1000) + index * 345) {
  }
};
...
void VerifyDBFromDB(std::string& truth_db_name) {
  ...
  void Run() {
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      } else if (name == "readuniwhilewriting") {
        num_threads++;  // Add extra thread for writing
        method = &Benchmark::ReadWhileWriting2;
      } else if (name == "readexpwhilewriting") {
        num_threads++;  // Add extra thread for writing
 ...
 private:
  ...
  bool YCSBOperation(DBWithColumnFamilies* db_with_cfh, const YCSB_Op& operation, const Slice& ivalue, std::string* ovalue) {
    ...
    static kv::TraceUniform seekrnd(333, 1, 100);
    bool res = false;
    char value_buffer[1024];
   
    std::unique_ptr<const char[]> key_guard;
    Slice key = AllocateKey(&key_guard);
    GenerateKeyFromInt(operation.key, FLAGS_num, &key);

    if (operation.type == kYCSB_Write) {
      res = db_with_cfh->db->Put(woption,key, ivalue).ok();
    } else if (operation.type == kYCSB_Read) {
      
      res = db_with_cfh->db->Get(roption, key, ovalue).ok();
    } else if (operation.type == kYCSB_Query) {
      auto* iter = db_with_cfh->db->NewIterator(roption);
      iter->Seek(key);
      if (iter->Valid()) {
        if (iter->key() == key) {
	    		  res = true;
      ...
      iter = nullptr;
        ...
        res = db_with_cfh->db->Get(roption, key, ovalue).ok();
        if (res) {
          db_with_cfh->db->Put(woption, key, ivalue);
        }
    }
  ...
  void YCSBReverse(ThreadState* thread) {
    if ((int64_t)ycsb_insertion_sequence.size() != FLAGS_num)
      RandomSequence(FLAGS_num, ycsb_insertion_sequence);

    // reverse insertion order, make the latest to frontier
  ...
  void YCSBLoad(ThreadState* thread) {
    ...
    for (uint64_t i = 0; i < len; ) {
      ...
      for (int64_t j = 0; j < entries_per_batch_; ++j) {
        ...
        // s = db_with_cfh->db->Put(woption, key, gen.Generate(value_size_));
  ...
  void YCSB(ThreadState* thread) {
    ...
    int64_t len = FLAGS_ycsb_ops_num;
    if (FLAGS_ycsb_type == kYCSB_E) {
      len = FLAGS_ycsb_ops_num / 4;
    }
    ...
    std::string ovalue;
    Duration duration(FLAGS_duration, FLAGS_ycsb_ops_num);
    DBWithColumnFamilies* db_with_cfh = SelectDBWithCfh(thread);

    thread->stats.Start(thread->tid);
    for (int64_t i = 0; i < len; ++i) {
      Slice ivalue = thread->gen.Generate(value_size_);
      bool op_res = YCSBOperation(db_with_cfh, ycsb_ops[total_ops], ivalue, &ovalue);
      if ((ycsb_ops[total_ops].type == kYCSB_Read || ycsb_ops[total_ops].type == kYCSB_ReadModifyWrite || ycsb_ops[total_ops].type == kYCSB_Query) && 
          op_res) {
        found++;
      }
      bytes += ycsb_ops[total_ops].type == kYCSB_Query ? (value_size_  + 16) * FLAGS_seek_nexts : (value_size_  + 16);
      thread->stats.FinishedOps(db_with_cfh, db_with_cfh->db,
                                1, kWrite);
      total_ops++;
    }
...
// optimization, so skip the check of the max level.
  ...
  void ReadAll(ThreadState* thread) {
    ...
    while (!duration.Done(1) && FLAGS_writes > 0) {
      ...
      int64_t key_rand = GetRandomKey(&rnd);
      key_rand = key_rand % (uint64_t)(FLAGS_range);
      GenerateKeyFromInt(key_rand, FLAGS_range, &key);
      read++;
      Status s;
      if (FLAGS_num_column_families > 1) {
  ...
  void MultiReadRandom(ThreadState* thread) {
    ...
    while (!duration.Done(1)) {
      ...
      for (int64_t i = 0; i < entries_per_batch_; ++i) {
        GenerateKeyFromInt(GetRandomKey(&thread->rand), FLAGS_range, &keys[i]);
      }
  ...
  void DoDelete(ThreadState* thread, bool seq) {
    ...
    while (!duration.Done(entries_per_batch_)) {
      ...
      batch.Clear();
      for (int64_t j = 0; j < entries_per_batch_; ++j) {
        const int64_t k = seq ? i + j : (thread->rand.Next() % FLAGS_range);
        GenerateKeyFromInt(k, FLAGS_range, &key);
        batch.Delete(key);
      }
  ...
  void BGWriter(ThreadState* thread, enum OperationType write_merge) {
    ...
    while (true) {
      ...
      GenerateKeyFromInt(thread->rand.Next() % FLAGS_range, FLAGS_range, &key);
      Status s;
  ...
  void BGWriter2(ThreadState* thread, enum OperationType write_merge) {
    // Special thread that keeps writing until other threads are done.
    RandomGenerator gen;
    int64_t bytes = 0;
  ...
  void UpdateRandom(ThreadState* thread) {
    ...
    while (!duration.Done(1)) {
      DB* db = SelectDB(thread);
      GenerateKeyFromInt(thread->rand.Next() % FLAGS_range, FLAGS_num, &key);
